...
create_random_na = function(
  ...
  where_na = withr::with_seed(seed, code = {
    n_na = seq(1, x, by = n_by)
    tmp_na = vector("list", length = (length(n_na)^2 * nrep^2))
    save_loc = 1
    for (x_na in n_na) {
      for (xrep in seq(1, nrep)) {
        for (y_na in n_na) {
          for (yrep in seq(1, nrep)) {
            tmp_na[[save_loc]] = c(sample(xloc, x_na), sample(yloc, y_na))
            save_loc = save_loc + 1
            #message(save_loc)
          }
        }
