...
##' @author rmflight
##' @export
create_lc_samples <- function(seed) {
  base_sample = withr::with_seed(seed, code = {
    rlnorm(1000, meanlog = 1, sdlog = 0.5)
  })
  rep_data = withr::with_seed(seed, code = {
    add_uniform_noise(2, base_sample, 0.2)
  })
  rep_data
}

left_censor_correlate = function(
  lc_samples,
  cut_values = seq(0, 1.5, by = 0.1)
) {
  censor_cor = purrr::map_df(cut_values, function(in_cut) {
    ...
    data.frame(
      ...
      which = c("ici", "pearson", "kendall", "pearson_0", "kendall_0"),
      n_na = n_na,
      cutoff = in_cut
    )
  })
...
lt_left_censor_correlate = function(
  lc_samples,
  cut_values = seq(0, 1.5, by = 0.1)
) {
  censor_cor = purrr::map_df(cut_values, function(in_cut) {
...
random_censor_correlate = function(
  lc_samples,
  n_na = seq(0, 250, 50),
  nrep = 100
) {
