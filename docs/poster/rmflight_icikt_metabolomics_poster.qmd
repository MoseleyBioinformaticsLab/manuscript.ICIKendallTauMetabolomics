...
format:
  poster-typst: 
    size: "48x36"
    poster-authors: "R.M. Flight, P.S. Bhatt, H.N.B. Moseley"
    departments: "University of Kentucky Markey Cancer Center"
    institution-logo: "./images/ukmarkeylogo.png"
    footer-text: "https://pypi.org/project/icikt/"
    footer-url: "https://moseleybioinformaticslab.github.io/ICIKendallTau"
    footer-emails: "robert.flight@uky.edu"
    footer-color: "ebcfb2"
    keywords: ["Missing Not At Random", "Left-Censored", "Correlation", "Metabolomics"]
    keep-typ: true
---

# Missing Values in Metabolomics Data Are Not Random
...
Kendall-tau correlation is the easiest correlation to manipulate due to the way it encodes the relationships via pairwise comparisons of the ranks of data points by concordance and discordance of the **pairs** of data points.
...
The second case, where we leave ties resulting from points with missing X and Y, we refer to as the "global" ICI-Kt correlation.
...
Overall, the ICI-Kt and the normal Kendall-tau replacing missing with zero have the desirable characteristics of maintaining the correlation with growing left-centered missing **while sharply dropping the correlation** with growing random missingness (@fig-mnar-mar C).
In a naive treatment of the left-centered missing data, if the values below the cutoff are set to missing followed by log-transforming the values and subsequently setting missing values to 0, then the Kendall-tau correlation replacing missing with 0 will show some very odd patterns at low intensity cutoffs due to the introduction of discordant pairs (@fig-mnar-mar B, center panel).
...
For each level of possible missingness introduced by changes in the dynamic range, correlations across all samples were calculated using all values (reference), as well as after missingness was added (trimmed), and using Pearson correlation with global imputation (Pearson Imputed), or ICI-Kt.
...
We evaluated the removal of outliers after sample-sample calculation using the various correlation methods and then removal of any outlier samples based upon median correlations to all other samples in the same set of SSF, calculation of moderated F-test statistics and adjusting of p-values, and then counting how many metabolites had an adjusted-p-value $\le 0.05$, as a fraction of the total number of measured metabolites.
@fig-mean-stats shows that ICI-Kt completeness (icikt_complete) and ICI-Kt (icikt) are ranked first and third by mean fraction determined as significant, followed by Pearson correlation using log-transformed values (pearson_log1p).
...
![**A**: Mean and standard deviation of the fraction total that is considered significant for each method across analyses for **high** fractions ($\ge 0.3$). **B**: Mean and standard deviation of the fraction total that is considered significant for each method across analyses for **low** fractions ($\le 0.3$). **C**: T-test results from comparison of significant fraction total for a method with the original fraction.](./images/combined_stats_label.png){#fig-mean-stats width=100%}
...
1. One or more SSF group with 5 or more replicates;
1. Contained at least one missing value (parsing to numeric returned NA or 0);
1. Abundance data could be directly parsed to a matrix;
1. Only one species was measured;
1. The sample IDs between SSF and the metabolites data block matched.
...
For consistency, we replaced all zero values with `NA`, and then either left them as `NA` or changed them to zero as appropriate for the method.

* ICI-Kendall-tau using `NA` (icikt); 
* and then scaled (multiplied) by a completeness metric (icikt_completeness);
* Kendall-tau using `NA`, and then using pairwise-complete-observations (kt_base);
...
* Pearson, with zeros replacing `NA`, and then a $log(x)$ transform, and setting infinite values to `NA` values, using pairwise-complete-observations (pearson_log).

## Detecting and Removing Possible Outliers

For outlier detection, median sample-sample correlations within each SSF is calculated, and $log(1 - cor_{median})$ calculated to transform it into a score. Then outliers are determined using *grDevices::boxplot.stats*, which by default are at 1.5X the whiskers in a box-and-whisker plot.
...
ICI-Kt is available in the R package `ICIKendallTau` from https://moseleybioinformaticslab.github.io/ICIKendallTau and the python package `icikt` from https://pypi.org/project/icikt/.
...
We also thank Travis Thompson for providing repaired mwTab files, as well as the Metabolomics Workbench for access to deposited metabolomics datasets.
