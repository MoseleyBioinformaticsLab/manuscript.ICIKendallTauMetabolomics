...
egfrgenotype_table = compare_outlier_tables_bold(
  egfrgenotype_single,
  compare_egfrgenotype,
  "pearson_log1p",
  map_method = other_method
)
egfrgenotype_table |>
  font(fontname = "Calibri", part = "all") |>
  fontsize(size = 11, part = "header") |>
...
all_adeno2 = dplyr::left_join(
  new_adeno,
  all_adeno,
  by = c("old_sample" = "sample_id")
)
adenocarcinoma_single = dplyr::left_join(
  adenocarcinoma_single,
  new_adeno,
  by = c("sample_id" = "old_sample")
)
adenocarcinoma_single = adenocarcinoma_single |>
  dplyr::mutate(sample_id = sample_id.y, sample_id.y = NULL)
...
adenocarcinoma_table = compare_outlier_tables_bold(
  adenocarcinoma_single,
  compare_adeno,
  "pearson_log1p",
  map_method = other_method
...
```{r}
#| label: nsclc_table
nsclc_table = compare_outlier_tables_bold(
  nsclc_single,
  compare_egfrgenotype,
  "pearson_log1p",
  map_method = other_method
...
ratstamina_table = compare_outlier_tables_bold(
  ratstamina_single,
  compare_egfrgenotype,
  "pearson_log1p",
  map_method = other_method
...
ggplot(
  all_yeast2,
  aes(x = method, y = med_cor, color = outlier, group = method)
) +
  geom_sina() +
...
ggplot(
  all_yeast2,
  aes(x = keep_num, y = med_cor, color = outlier, group = keep_num)
) +
  geom_sina() +
  scale_color_manual(values = outlier_colors) +
...
all_egfrgenotype_method = add_method(
  all_egfrgenotype,
  map_method = other_method
) %>%
  dplyr::filter(sample_class %in% egfrgenotype_classes)
ggplot(
  all_egfrgenotype_method,
  aes(x = method, y = med_cor, color = outlier, group = method)
) +
  geom_sina() +
  scale_color_manual(values = outlier_colors) +
...
egfrgenotype_diffs = calculate_differences(
  egfrgenotype_summary,
  groups = c("wt", "null")
)
...
nsclc_diffs = calculate_differences(
  nsclc_summary,
  groups = c("FSN10115:cancer", "FSN10115:non-cancer")
)
...
ratstamina_diffs = calculate_differences(
  ratstamina_summary,
  groups = c("Ad lib:High", "Ad lib:Low")
)
...
save(
  ...
  nsclc_table,
  ratstamina_table,
  adenocarcinoma_ratios,
  file = here::here("docs/supp_stuff.rda")
)
```
