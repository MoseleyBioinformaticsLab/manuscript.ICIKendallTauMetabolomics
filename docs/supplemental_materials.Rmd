...
tar_load(c(
  ...
  vl_na_perc_graph,
  performance_plot_features,
  performance_plot_parallel,
...
`r supp_figure_count$label_text("performance-features")`. 
...
`r supp_figure_count$label_text("performance-parallel")`.
**(A)**: Time in seconds as the number of compute cores are increased.
**(B)**: Additional memory used as the number of compute cores are increased.
