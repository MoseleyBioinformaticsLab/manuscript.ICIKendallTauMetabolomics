...
Random variance is added to one of the two samples by drawing values from a uniform distribution over -0.5 to 0.5, and adding the values to the original sample, and sorting them again to maintain a correlation of 1 or -1 for Kendall-$\tau$ correlation.
...
a maximum metabolite feature abundance $\ge$ 20 to exclude log-transformed values and low dynamic range datasets, as we didn't want to troubleshoot code for un-transforming log-transformed values;
...
Likewise, the negative ICI-Kt correlations appear unaffected; however, the scale of changes seen in the negative Pearson correlations, if present in the negative ICI-Kt correlations, might simply be obscured the changes due to missingness that are orders of magnitude larger.
